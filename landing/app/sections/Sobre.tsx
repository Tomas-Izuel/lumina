...
export default function Sobre() {
  return (
    <div id="sobre" style={{ maxWidth: 1180, margin: "0 auto", padding: "80px 28px", position: "relative", zIndex: 10 }}>
      <div data-reveal className="sobre-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "center", padding: 48, borderRadius: 26, background: "linear-gradient(135deg,rgba(184,172,216,0.22),rgba(238,219,226,0.28))", border: "1px solid rgba(255,255,255,0.6)", boxShadow: "0 28px 64px -34px rgba(80,60,120,0.4)" }}>
        <div>
          ...
          <h2 className="font-serif sec-h2" style={{ fontWeight: 400, fontSize: 40, lineHeight: 1.1, letterSpacing: "-0.018em", margin: "14px 0 16px", color: "#241f30" }}>IA aplicada a vender y alquilar más rápido.</h2>
          <p style={{ fontSize: 16, lineHeight: 1.6, color: "#544d62", margin: 0 }}>Lumina construye herramientas de inteligencia artificial para acelerar la conversión en real estate. El tour virtual es nuestro primer producto — el primero de muchos pensados para que brokers e inmobiliarias cierren operaciones antes.</p>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={benefitRow}>
            <span style={iconBox}><svg {...ico}><path d="M3 17l6-6 4 4 8-8" /><path d="M17 7h4v4" /></svg></span>
            <div>
              <div style={{ fontWeight: 600, color: "#2a2433", fontSize: 15 }}>Más visitas calificadas</div>
              <div style={{ fontSize: 14, color: "#6b6478" }}>El cliente recorre antes de pisar el inmueble.</div>
            </div>
          </div>
          <div style={benefitRow}>
          ...
          <div style={benefitRow}>
            ...
            <div>
              <div style={{ fontWeight: 600, color: "#2a2433", fontSize: 15 }}>Calidad consistente</div>
              <div style={{ fontSize: 14, color: "#6b6478" }}>Cada tour con el mismo estándar profesional.</div>
            </div>
          </div>
        </div>
      </div>
